...
class PlayerManager @Inject constructor(
    ...
    fun createMediaSession(service: PlaybackService): MediaSession {
        ...
        exoPlayer.addListener(
            object : Player.Listener {
                override fun onIsPlayingChanged(isPlaying: Boolean) = persistSnapshot(isPlaying)
                ...
                override fun onShuffleModeEnabledChanged(shuffleModeEnabled: Boolean) =
                    persistSnapshot(exoPlayer.isPlaying)
                override fun onRepeatModeChanged(repeatMode: Int) = persistSnapshot(exoPlayer.isPlaying)
            },
        )

        val session = MediaSession.Builder(service, exoPlayer).build()
    ...
    private fun persistSnapshot(isPlaying: Boolean) {
        val player = player ?: return
        if (player.mediaItemCount == 0) {
            scope.launch { recoveryStore.clear() }
            return
        }

        val songIds = buildList {
            for (index in 0 until player.mediaItemCount) {
                val id = player.getMediaItemAt(index).mediaId.toLongOrNull() ?: continue
                add(id)
            }
        }
    ...
    private suspend fun restorePlayback(player: ExoPlayer) {
        val snapshot = recoveryStore.load() ?: return
        val songs = localMusicDataSource.findSongsByIds(snapshot.songIds)
        if (songs.isEmpty()) {
            recoveryStore.clear()
            return
        }
        ...
        val orderedSongs = snapshot.songIds.mapNotNull { songsById[it] }
        if (orderedSongs.isEmpty()) {
            recoveryStore.clear()
            return
        }

        player.setMediaItems(orderedSongs.map { it.toMediaItem() })
        player.repeatMode = snapshot.repeatMode.toPlayerRepeatMode()
        player.shuffleModeEnabled = snapshot.shuffleEnabled

        val safeIndex = snapshot.currentIndex.coerceIn(0, orderedSongs.lastIndex)
        player.seekTo(safeIndex, snapshot.positionMs.coerceAtLeast(0L))
        player.prepare()
        if (snapshot.wasPlaying) {
            player.play()
        }
        Logger.i(TAG, "Restored playback queue with ${orderedSongs.size} songs")
    }
